...
from models.custom_reward import TransformerRewardPredictor_v2
...
def sample_trajectory(memory, n_trajectories=128):
    ...
    next_states = np.array(list(samples.next_states)).reshape(n_trajectories, args.num_steps, n_agents, -1)[:, :, :, :-((n_agents-1)*2)].reshape(n_trajectories, args.num_steps, -1)
