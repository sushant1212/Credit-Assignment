...
def sample_trajectory(memory, n_trajectories=128):
    ...
    next_states = np.array(list(samples.next_states)).reshape(n_trajectories, args.num_steps, -1)
...
def sample_and_pred(memory, model:CustomTimeRewardNet, batch_size, n_agents, n_trajectories=128):
    ...
    y_time_hat = model.predict_reward(x_train_tensor.to(device), reward_tensor.to(device))
