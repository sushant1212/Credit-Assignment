...
sys.path.append(str(os.path.join(os.path.abspath(__file__), "../models/")))
...
from models.custom_reward import CustomTimeRewardNet 
from models.custom_reward import TransformerRewardPredictor_v2, TransformerRewardPredictor_v4
...
def sample_trajectory(memory, n_trajectories=128):
    ...
    next_states = np.array(list(samples.next_states)).reshape(n_trajectories, args.num_steps, n_agents, -1)[:, :, :, :-((n_agents-1)*2)].reshape(n_trajectories, args.num_steps, n_agents, -1)
    ...
    y_train_tensor = torch.from_numpy(episode_reward).unsqueeze(-1).float().contiguous()
...
def sample_and_pred(memory, model:CustomTimeRewardNet, batch_size, n_agents, n_trajectories=128):
    sample_traj = random.sample(memory, n_trajectories)
    samples = Transition_e(*zip(*sample_traj))

    next_states = np.array(list(samples.next_states)).reshape(n_trajectories, args.num_steps, n_agents, -1)[:, :, :, :-((n_agents-1)*2)].reshape(n_trajectories, args.num_steps, n_agents, -1)
    true_rewards = np.array(list(samples.rewards)).reshape(n_trajectories, args.num_steps, n_agents).transpose(0, 2, 1)
    true_rewards = np.sum(true_rewards[:, 0, :], axis=-1).reshape(-1, 1)
    x_train_tensor = torch.from_numpy(next_states).float().contiguous()
    reward_tensor = torch.from_numpy(true_rewards).float().contiguous()
    predicted_ep_reward, temporal_attention_weights, agent_attention_weights = model(x_train_tensor.to(device))
    agent_attention_weights = torch.mean(agent_attention_weights.squeeze(-2), dim=2)
    temporal_attention_weights = torch.mean(temporal_attention_weights.squeeze(), dim=1)
    
    if args.use_true_reward:
        y_time_hat = temporal_attention_weights * (reward_tensor.to(device))
    else:
        y_time_hat = temporal_attention_weights * predicted_ep_reward.to(device)
        
    pred_rewards = (y_time_hat.unsqueeze(-1) * agent_attention_weights).detach().cpu().numpy()
...
# reward_model = CustomTimeRewardNet(
...
reward_model = TransformerRewardPredictor_v4 (
    n_agents=n_agents,
    obs_input_dim=(env.observation_space[0].shape[0]-(n_agents-1)*2),
    num_heads=4,
...
def make_train_step(model, loss_fn, optimizer):
    ...
    def train_step(x, y, z):
        ...
        model.train()
        # Makes predictions
        yhat, temporal_attention_weights, agent_attention_weights = model(x)
        temporal_attention_weights = torch.mean(temporal_attention_weights.squeeze(), dim=1)
        agent_attention_weights = torch.mean(agent_attention_weights.squeeze(-2), dim=2)
        if args.use_true_reward:
            y_time_hat = temporal_attention_weights * (z.to(device))
        else:
            y_time_hat = temporal_attention_weights * yhat.to(device)

        # Computes loss
        loss = loss_fn(y, yhat)
        loss_2 = loss_fn(z, y_time_hat)
        loss_3 = (20 * torch.mean(torch.var(y_time_hat, dim=1)) ) if args.use_variance_loss else torch.tensor(0.0).to(device)
