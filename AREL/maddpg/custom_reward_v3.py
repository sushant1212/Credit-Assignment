...
from models.custom_reward import TransformerRewardPredictor_v2, TransformerRewardPredictor_v3
...
def sample_and_pred(memory, model:CustomTimeRewardNet, batch_size, n_agents, n_trajectories=128):
    ...
    if args.use_true_reward:
        ...
        y_time_hat = attention_weights * predicted_ep_reward.to(device)
...
reward_model = TransformerRewardPredictor_v3 (
    n_agents=n_agents,
...
def make_train_step(model, loss_fn, optimizer):
    ...
    def train_step(x, y, z):
        ...
        attention_weights = torch.mean(attention_weights.squeeze(), dim=1)
        if args.use_true_reward:
            y_time_hat = attention_weights * (z.to(device))
        else:
            y_time_hat = attention_weights * yhat.to(device)
